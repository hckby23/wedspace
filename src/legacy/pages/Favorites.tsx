...
const Favorites: React.FC = () => {
  ...
  const removeFromFavorites = (type: 'venue' | 'vendor', id: string) => {
    if (type === 'venue') {
      ...
      if (typeof window !== 'undefined') window.alert("The venue has been removed from your favorites");
    } else {
      setFavoriteVendors(favoriteVendors.filter(vendor => vendor.id !== id));
      if (typeof window !== 'undefined') window.alert("The vendor has been removed from your favorites");
    }
  };
  ...
  const bookItem = (type: 'venue' | 'vendor', id: string) => {
    ...
    if (typeof window !== 'undefined') window.alert(`You've booked this ${type}! Check your planning page for updates.`);
  };
                  ...
                  {filteredVendors.map(vendor => (
                    <div key={vendor.id} className="flex flex-col">
                      <div className="relative flex-grow">
                        ...
                        <VendorCard vendor={vendor} showFavoriteButton={false} />
                      </div>
                      <Button 
                        className="w-full mt-2 bg-wed hover:bg-wed/90"
